...
class EditImageViewModel(private val editImageRepository: EditImageRepository) : ViewModel() {
    ...
    private var imagePreviewDataState = MutableLiveData<ImagePreviewDataState>()
    ...
    private fun emitImagePreviewUiState(
        isLoading: Boolean = false, bitmap: Bitmap? = null, error: String? = null
    ) {
    ...
    data class ImagePreviewDataState(
        val isLoading: Boolean, val bitmap: Bitmap?, val error: String?
    )
    ...
    private fun emitImageFiltersUiState(
        isLoading: Boolean = false, imageFilters: List<ImageFilter>? = null, error: String? = null
    ) {
    ...
    fun saveFilteredImage(filteredBitmap: Bitmap){
        Coroutines.io {
            runCatching {
                emitSaveFilteredUiState(isLoading = true)
                editImageRepository.saveFilteredImage(filteredBitmap)
            }.onSuccess { savedImageUri->
                emitSaveFilteredUiState(uri = savedImageUri)
            }.onFailure {
                emitSaveFilteredUiState(error = it.message.toString())
            }
        }
    }

    private fun emitSaveFilteredUiState(
